...
try:
    cp = run([sys.executable,"./huff-compress.py","-s",symbolmodel,args.infile,'--bench'],check=True,timeout=timeout)
...
try:
    cp = run([sys.executable,"./huff-decompress.py",root +'.bin','--bench'],check=True,timeout=timeout)
